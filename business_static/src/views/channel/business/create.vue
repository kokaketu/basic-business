        ...
        <div class="filter-wrapper left-wrapper remove-style">
          <el-form
            ...
            <!-- <el-form-item label="环线位置" prop="loopLinePosition" required>
              <el-select
                v-model="baseInfo.loopLinePosition"
                placeholder="请选择环线位置">
                <el-option
                  class='select-option-selected'
                  v-for="item in constObj.loopLine"
                  :key="item.code"
                  :label="item.name"
                  :value="item.code">
                </el-option>
        ...
        <div class="filter-wrapper left-wrapper remove-style">
          <el-form
            ...
            <el-form-item label="地下楼层数" prop="groundFloorDown" required>
              <el-input v-model="baseInfo.groundFloorDown" placeholder="请输入地下楼层数，支持整数" oninput="value=value.replace(/[^0-9]/g, '')" type="text" />
            </el-form-item>
      ...
      <div class="new-block">
        <div class="filter-wrapper left-wrapper remove-style">
          <el-form :inline="true" ref="demand" :model="housing" size="small" label-width="116px">
            ...
            <el-form-item label="业态：" prop="layoutPositioning" v-if="baseInfo.assetLayout === '2'" required>
              <el-select
            ...
            <el-form-item label="行业：" prop="industry" v-if="baseInfo.assetLayout === '1' || baseInfo.assetLayout === '3'" required>
              <el-select
            ...
            <el-form-item label="最短租期" prop="minLeaseTerm" required>
              <!-- <el-input v-model="housing.minLeaseTerm" placeholder="请输入" oninput="value=value.replace(/^(0+)|[^\d]+/g,'')"></el-input>
              <span data-v-373b083b="" style="position: absolute;right: 10px;">年</span> -->
              <InputNumber
      ...
      <div class="new-block">
        <div class="filter-wrapper left-wrapper remove-style">
          <el-form :inline="true" size="small" label-width="130px" ref="picture">
            <el-form-item label="租赁单元实景图" prop="first_image_url" required>
...
import { getProjectList, getMessage, handelAddBusiness } from '@/http/business'
...
export default {
  ...
  methods: {
    handler ({BMap, map}) {
      ...
      //   function (r) {
      //     this.positionMap = {lat: r.point.lat, lng: r.point.lng}
      //   }
      // )
    },
    handelDragend (obj) {
    ...
    querySearchAsync (queryString, cb) {
      this.baseInfo = {assetName: this.baseInfo.assetName}
      this.buildingList = [{
        buildingName: '',
        buildingArea: '',
        standardFloorArea: '',
    ...
    add () {
      ...
      const refData = ['baseInfo', 'area', 'enterprise', 'xmpt', 'building', 'buildref', 'housing', 'demand', 'linkman', 'tenement']
      let resultArr = []
      refData.map(item => {
      ...
      if (resultArr.every(item => item)) {
        ...
        this.baseInfo.mainId = this.merchant.id
        let params = {
          ...this.housing,
