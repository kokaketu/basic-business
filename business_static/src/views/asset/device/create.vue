<template>
  <div id="device-create">
        ...
        <div class="filter-wrapper remove-style">
          <el-form :inline="true" :model="ruleForm3" ref="ruleForm3" :rules="rules3" size="small" label-width="150px">
            ...
            <el-form-item label="关联楼栋" prop="buildingId">
              <el-select v-model="ruleForm3.buildingId" placeholder="请选择">
                <el-option
                  class='select-option-selected'
            ...
            <el-form-item label="关联楼层" prop="floorId">
              <el-select v-model="ruleForm3.floorId" placeholder="请选择">
                <el-option
                  class='select-option-selected'
        ...
        <div class="content-wrapper content-tabs">
          <el-tabs v-model="activeName">
            ...
            <el-tab-pane label="附件信息" name="attachment">
              <!-- <el-table :data="ruleForm1.attachmentList">
                <el-table-column type="index" width="50px" label="序号"></el-table-column>
                <el-table-column label="附件名称">
                  <template slot-scope="scope">
                    <el-link type="primary" :href="scope.row.attachmentPath" target="_blank">{{scope.row.attachmentName}}</el-link>
                  </template>
                </el-table-column>
                <el-table-column width="60">
                  <template slot="header">
              ...
              <div class="accessory">
                <div class="list-accessory" v-for="(item, index) in ruleForm1.attachmentList" :key="item.uid">
                  <div class="left">
    ...
    <div class="btn-group">
      <el-button class="btn btn-default pull-right" style="margin-right: 0" @click.native="cancel">取消</el-button>
...
import { saveDevice, getSequence } from '@/http/device'
...
export default {
  ...
  activated () {
    ...
    getSequence({mainId: this.merchant.id}).then(res => {
      this.$set(this.ruleForm1, 'no', res.data)
    })
  },
  watch: {
    'ruleForm3.assetId': {
  ...
  methods: {
    handleRemove (file, fileList) {
      this.ruleForm1.attachmentList = fileList
    },
    ...
    fileChange (file, fileList) {
      ...
      http.post('/asset-fileservice-web/filesystem/upload', data).then(res => {
        if (res.code === '0') {
          this.ruleForm1.attachmentList.push({name: file.name, url: res.data, attachmentName: file.name, attachmentPath: res.data, attachmentType: 1})
    ...
    deleteFile (index) {
      this.ruleForm1.attachmentList.splice(index, 1)
    },
    save () {
      ...
      Promise.all([p1, p2, p3]).then(() => {
        ...
        params.supplierList = this.suppliers
        params.serviceRecordsList = this.serviceRecords
        saveDevice(params).then(res => {
          if (res.code === '0') {
            ...
            this.$router.push(`/asset/device/detail/${res.data.id}`)
          }
        })
      })
    },
