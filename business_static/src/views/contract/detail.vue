 ...
 * @LastEditTime: 2020-08-31 17:34:13
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \asset_management_static\src\views\contract\business\detail.vue
-->
<template>
...
<!--          <el-button :disabled="permission" class="btn pull-right" v-if="operation.restore" @click.native="reinstate">恢复</el-button>-->
      ...
      <div class="detailInfo">
        ...
        <KeyValue keytag='租赁单元' :valuetag="baseInfo.rentalUnits" />
      </div>
      <!-- 租约信息 -->
      <div class="wrapper-title">租约信息</div>
      <div class="detailInfo">
        <KeyValue keytag='建筑面积' :valuetag="detail.buildingArea" />
        <KeyValue keytag='套内面积' :valuetag="detail.innerArea" />
        ...
        <KeyValue keytag='装修开始日期' :valuetag="detail.decorationStartDate" />
        <KeyValue keytag='装修结束日期' :valuetag="detail.decorationEndDate" />
      </div>
      <div class="detailInfo" v-for="(item, index) in detail.freeDate" :key="index">
        <KeyValue keytag='免租期开始日期' :valuetag="item.start" />
        <KeyValue keytag='免租期结束日期' :valuetag="item.end" />
      </div>
      <div class="wrapper-title">租金条款</div>
      <div class="detailInfo">
        <KeyValue keytag='计租方式' :valuetag="rentTerms.chargeType" />
        <KeyValue keytag='收费形式' :valuetag="rentTerms.chargeForm" />
        <KeyValue keytag='最迟缴款日' :valuetag="rentTerms.lastPayDate == 32 ? '每月最后1天' : rentTerms.lastPayDate" />
        <KeyValue keytag='支付周期' :valuetag="rentTerms.payCycle" />
      </div>
      <!-- 租金条款 -->
      <div class="block">
        <div class="tabs">
        ...
        <div class="tab-wrapper table-input">
          <el-table
            ...
            <el-table-column
              ...
              label="首期总金额(元)(含税)">
              <template slot-scope="scope">
                <el-input disabled v-model="scope.row.receivableAmount"></el-input>
              </template>
            </el-table-column>
            <el-table-column
              prop="taxRate"
        ...
        <div class="tab-wrapper table-input"
          v-show="rentTerms.cycles &&  rentTerms.chargeTypeCode == '1'">
          <el-table
            :data="rentTerms.cycles"
            border
            style="width: 1000px;">
            <el-table-column
              prop="chargeDate"
              ...
              width="230">
              <template slot-scope="scope">
                <div>{{scope.row.chargeStartDate}} ~ {{scope.row.chargeEndDate}}</div>
              </template>
            </el-table-column>
           <el-table-column
              prop="monthAmount"
              label="含税月租金(元)">
              <template slot-scope="scope">
                <el-input disabled v-model="scope.row.monthAmount" placeholder="请输入"></el-input>
              </template>
            </el-table-column>
            <el-table-column
              prop="taxRate"
              label="税率(%)">
              <template slot-scope="scope">
                <el-input disabled v-model="scope.row.taxRate" placeholder="请输入"></el-input>
              </template>
            </el-table-column>
            <el-table-column
              ...
              label="不含税月租金(元)">
              <template slot-scope="scope">
                {{scope.row.monthAmountNoTax}}
              </template>
            </el-table-column>
          </el-table>
        </div>
        <div class="tab-wrapper table-input"
          v-show="assetLayout == 2 && rentTerms.cycles && rentTerms.chargeTypeCode == '2'">
          <el-table
            :data="rentTerms.cycles"
            border
            style="width: 1000px;">
              ...
              <template slot-scope="scope">
                <div class="cell-range-group"
                  ...
                  <div class="cell-range-wrapper clearfix">
                    <el-input disabled v-model="item.salesStart" placeholder="请输入"></el-input>
                    <p class="center">至</p>
                    <el-input disabled v-model="item.salesEnd" placeholder="请输入"></el-input>
                  </div>
                </div>
              </template>
            </el-table-column>
            <el-table-column
              ...
              <template slot-scope="scope">
                <div class="cell-range-group"
                  v-for="(item, index) in scope.row.steps"
                  :key="index">
                    <el-input class="single" disabled v-model="item.discountRate" placeholder="请输入">
                    </el-input>
                </div>
              </template>
            </el-table-column>
        ...
        <div class="tab-wrapper table-input"
          v-show="assetLayout == 2 && rentTerms.cycles && rentTerms.chargeTypeCode == '3'">
          <el-table
            :data="rentTerms.cycles"
            border
            style="width: 1000px;">
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <div class="cell-range-group"
                  v-for="(item, index) in scope.row.steps"
                  :key="index">
                    <el-input disabled class="single" v-model="item.discountRate" placeholder="请输入">
                    </el-input>
                </div>
              </template>
            </el-table-column>
            ...
            <el-table-column
              ...
              label="不含税月租金(元)">
              <template slot-scope="scope">
                <p class="table-cell-disable">{{scope.row.monthAmountNoTax}}</p>
              </template>
            </el-table-column>
          </el-table>
        </div>
      ...
      <div class="detailInfo">
        <KeyValue keytag='计租方式' :valuetag="propertyTerms.chargeType" />
        <KeyValue keytag='收费形式' :valuetag="propertyTerms.chargeForm" />
        <KeyValue keytag='最迟缴款日' :valuetag="propertyTerms.lastPayDate == 32 ? '每月最后1天' : propertyTerms.lastPayDate" />
        <KeyValue keytag='支付周期' :valuetag="propertyTerms.payCycle" />
      </div>
      <div class="block">
        <div class="tabs">
          <ul class="clearfix">
  ...
  methods: {
    ...
    getDetail () {
      ...
      detail(this.contractId).then(res => {
        ...
        this.attachmentList = res.data.attachmentStrList
        let data = res.data
        this.detail = data
    ...
    edit () {
      ...
      this.$router.push(`/contract/edit/${params}`)
    }
  },
  components: {
    KeyValue,
