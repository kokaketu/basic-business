 ...
 * @LastEditTime: 2020-09-04 16:19:19
      ...
      <div class="block">
        <el-form
          ...
          <el-form-item
            label="行业"
            required
            v-show="baseInfo.companySort == '1' || baseInfo.companySort == '3'"
    ...
    <div class="wrapper company-info">
      ...
      <div class="block">
        <el-form :inline="true" ref="companyPhone" :model="companyInfo" size="small" label-width="100px">
          ...
          >
            <el-input v-model="companyInfo.companyPhone" oninput="if(value.length>13)value=value.slice(0,13)" placeholder="请输入公司电话"></el-input>
          </el-form-item>
          <el-form-item label="统一社会信用代码" label-width="130px">
            <el-input v-model="companyInfo.socialCreditCode" oninput="if(value.length>32)value=value.slice(0,32)" placeholder="请输入统一社会信用代码"></el-input>
          </el-form-item>
          <br/>
        ...
        <el-table
          ...
          <!-- <el-table-column>
            ...
            <template slot-scope="scope">
              <el-input v-model="scope.row.brandCode" placeholder="请输入"></el-input>
            </template>
          </el-table-column> -->
          <el-table-column>
            <template  slot="header">
            ...
            <template slot-scope="scope">
              <el-input v-model="scope.row.brandName" oninput="if(value.length>16)value=value.slice(0,16)" placeholder="请输入"></el-input>
            </template>
          </el-table-column>
          <el-table-column
            ...
            label="品牌名称(英)">
            <template slot-scope="scope">
              <el-input v-model="scope.row.brandEnglishName" oninput="if(value.length>20)value=value.slice(0,20)" placeholder="请输入"></el-input>
            </template>
          </el-table-column>
          <el-table-column>
            <template  slot="header">
      ...
      <div class="block table-input">
        <el-table
          ...
          <el-table-column>
            ...
            <template slot-scope="scope">
              <el-input v-model="scope.row.contactPhone" oninput="if(value.length>11)value=value.slice(0,11)" placeholder="请输入"></el-input>
            </template>
          </el-table-column>
          <el-table-column
            prop="contactMail"
            label="邮箱">
            <template slot-scope="scope">
              <el-input v-model="scope.row.contactMail" placeholder="请输入" oninput="if(value.length>128)value=value.slice(0,128)"></el-input>
            </template>
          </el-table-column>
          <el-table-column
            prop="contactAddress"
            label="通讯地址">
            <template slot-scope="scope">
              <el-input v-model="scope.row.contactAddress" oninput="if(value.length>512)value=value.slice(0,512)" placeholder="请输入"></el-input>
            </template>
          </el-table-column>
          <el-table-column
            width="60"
...
import {initCompany, addCompany} from '@/http/rentControl'
...
export default {
  data () {
    return {
      ...
      baseInfoRules: {
        ...
        //   { required: true, message: '请输入租户编号', trigger: 'change' }
        // ],
        companyName: [
          { required: true, message: '请输入租户全称', trigger: 'blur' },
          { required: true, message: '请输入租户全称', trigger: 'change' }
        ],
        companyShortName: [
          { required: true, message: '请输入租户简称', trigger: 'blur' },
          { required: true, message: '请输入租户简称', trigger: 'change' }
        ],
        assetId: [
  ...
  methods: {
    ...
    add () {
      ...
      let params = {
        ...
        'id': '',
        'assetName': this.projectNameList.filter(({id}) => Number(id) === Number(this.baseInfo.assetId))[0].assetName,
      ...
      addCompany(params).then(res => {
        if (res.code === '0') {
          ...
          this.$router.push(`/rentcontrol/tenant/detail/${res.data}`)
  ...
  activated () {
    ...
    this.initCompany()
    // 从客户跳转过来的 将客户数据带入租户表单
    if (this.$route.params.source === 'ibClient') {
      console.log(this.$route.params)
      this.baseInfo.clientId = this.$route.params.id
      this.baseInfo.assetId = this.$route.params.assetId
      this.baseInfo.companyName = this.$route.params.fullName
      this.baseInfo.companyAddress = this.$route.params.address
      this.baseInfo.companyShortName = this.$route.params.abbreviation
      this.baseInfo.cityCode = this.$route.params.cityCode
      this.baseInfo.cityName = this.$route.params.cityName
      this.baseInfo.provinceCode = this.$route.params.provinceCode
      this.baseInfo.provinceName = this.$route.params.provinceName
      this.baseInfo.districtCode = this.$route.params.districtCode
      this.baseInfo.districtName = this.$route.params.districtName
      this.selectedCity = {
      ...
      this.baseInfo.companyType = this.$route.params.clientType
      this.brandList = [{
        id: '',
        companyId: '',
        // brandCode: '',
